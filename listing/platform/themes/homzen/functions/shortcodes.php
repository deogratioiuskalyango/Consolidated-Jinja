...
Event::listen(RouteMatched::class, function (): void {
    ...
    Shortcode::register(
        'platform-hub',
        __('Platform Hub'),
        __('Unified rental, room booking, and hardware marketplace section.'),
        function (ShortcodeCompiler $shortcode) {
            $stats = [
                'rentals' => is_plugin_active('real-estate') && Schema::hasTable('re_properties')
                    ? DB::table('re_properties')->count()
                    : 0,
                'rooms' => is_plugin_active('hotel') && Schema::hasTable('ht_rooms')
                    ? DB::table('ht_rooms')->count()
                    : 0,
                'products' => is_plugin_active('ecommerce') && Schema::hasTable('ec_products')
                    ? DB::table('ec_products')->where('is_variation', 0)->count()
                    : 0,
                'stores' => is_plugin_active('marketplace') && Schema::hasTable('mp_stores')
                    ? DB::table('mp_stores')->count()
                    : 0,
            ];

            return Theme::partial('shortcodes.platform-hub.index', compact('shortcode', 'stats'));
        }
    );
