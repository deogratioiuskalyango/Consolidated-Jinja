        ...
        return array_merge($columns, [
            ...
            FormattedColumn::make('amount')
                ...
                ->renderUsing(function (FormattedColumn $column) {
                    $amount = $column->getItem()->amount;
                    $class = $amount >= 0 ? 'text-success' : 'text-danger';
                    $prefix = $amount >= 0 ? '+' : '';

                    return Html::tag('span', $prefix . format_price(abs($amount) / 100), [
                        'class' => $class . ' fw-bold',
                    ]);
                })
                ->alignEnd(),
            FormattedColumn::make('balance_after')
                ...
                ->renderUsing(fn (FormattedColumn $column) => format_price($column->getItem()->balance_after / 100))
                ->alignEnd(),
