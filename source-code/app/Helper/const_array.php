...
{
    $output = [
        // PNG — standard + legacy Windows variant
        'image/png',
        'image/x-png',
        // JPEG — all common variants
        'image/jpeg',
        'image/jpg',
        'image/pjpeg',
        'image/jfif',
        ...
        // Fallback for browsers/OS that report a generic type
...
function getPaymentServiceClass($input = null)
{
    $output = array(
        ...
        PESAPAL => 'App\Services\Payment\PesapalService',
    );
    if (is_null($input)) {
        return $output;
    } else {
...
if (!function_exists("getGatewaySupportedCurrencies")) {
    function getGatewaySupportedCurrencies($gateway = null)
    {
        $supported_currencies = array(
            ...
            PESAPAL => ['KES', 'UGX', 'TZS', 'USD', 'GBP', 'EUR', 'RWF', 'ZMW', 'MWK', 'GHS', 'ZAR'],
        );
...
function getRoleName($input = null)
{
    $output = [
        ...
        USER_ROLE_SHAREHOLDER  => 'Shareholder',
    ];
