...
if (!function_exists('getLayout')) {
    function getLayout()
    {
        ...
        return $output[auth()->user()->role] ?? 'owner';
    }
}
...
if (!function_exists('getOwnerUserId')) {
    function getOwnerUserId($user_id = null)
    {
        ...
        if (is_null($user_id) && session()->has('active_role')) {
            $assignment = \App\Models\SystemUserRole::where('user_id', $user->id)
                ->where('role_slug', session('active_role'))
