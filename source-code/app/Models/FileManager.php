...
class FileManager extends Model
{
    ...
    public function upload($to, $file, $name = '')
    {
        try {
            ...
            $symfony_mime  = $file->getMimeType(); // Symfony finfo-based detection
            $mime_allowed  = allowMimes();
            if ((!in_array($mime_type, $mime_allowed) && !in_array($symfony_mime, $mime_allowed))
                || !in_array($extension, allowExtensions())) {
                throw new Exception('Invalid File: type ' . $mime_type . ' / ext ' . $extension);
            }
