    ...
    public function handle(Request $request, Closure $next, ...$guards)
    {
        ...
        foreach ($guards as $guard) {
            if (Auth::guard($guard)->check()) {
                ...
                } elseif ($role == USER_ROLE_OWNER || $role == USER_ROLE_TEAM_MEMBER) {
                    return redirect(route('owner.dashboard'))->with('success', __('Login Successful'));
                } elseif ($role == USER_ROLE_TENANT) {
                    return redirect(route('tenant.dashboard'))->with('success', __('Login Successful'));
                } elseif ($role == USER_ROLE_MAINTAINER) {
                    return redirect(route('maintainer.dashboard'))->with('success', __('Login Successful'));
                } elseif ($role == USER_ROLE_SHAREHOLDER) {
                    return redirect(route('shareholder.dashboard'))->with('success', __('Login Successful'));
                } elseif ($role == USER_ROLE_ACCOUNTANT) {
                    return redirect(route('accountant.dashboard'))->with('success', __('Login Successful'));
                } else {
                    Auth::logout();
