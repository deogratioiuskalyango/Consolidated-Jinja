...
class SettingController extends Controller
{
    ...
    {
        $data['pageTitle']                  = __('WhatsApp Setting');
        $data['subWhatsappSettingActiveClass'] = 'active';
        return view('admin.setting.whatsapp-setting')->with($data);
    }
    ...
    {
        $phone = $request->query('phone') ?? auth()->user()->whatsapp_number ?? null;

        if (!$phone) {
            return redirect()->route('admin.setting.whatsapp.setting')
                ->with('error', __('Please enter a phone number to test. Add ?phone=628xxx to the URL, or set a WhatsApp number on your profile.'));
        }

        $wa  = app(WhatsAppService::class);
        $ok  = $wa->send($phone, '✅ Test message from *' . getOption('app_name', 'Zaiproty') . '*. WhatsApp integration is working!');

        if ($ok) {
            return redirect()->route('admin.setting.whatsapp.setting')
                ->with('success', __('Test message sent to') . ' ' . $phone);
        }

        return redirect()->route('admin.setting.whatsapp.setting')
            ->with('error', __('Failed to send test message. Check your OpenWA settings and logs.'));
    }
