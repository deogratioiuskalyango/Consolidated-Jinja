...
class KycConfigService
{
    ...
    public function getActiveByTenantId($id)
    {
        ...
        $configs = KycConfig::query()
            ->where(function ($query) use ($id, $ownerUserId) {
                // Owner-created configs visible to this tenant (or all tenants)
                $query->where(function ($q) use ($id, $ownerUserId) {
                    $q->where('owner_user_id', $ownerUserId)
                      ->where(function ($q2) use ($id) {
                          $q2->where('tenant_id', $id)->orWhereNull('tenant_id');
                      });
                })
                // Global system configs (owner_user_id IS NULL — seeded defaults)
