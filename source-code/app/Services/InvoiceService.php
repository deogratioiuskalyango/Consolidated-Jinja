...
class InvoiceService
{
    ...
    public function store($request)
    {
        ...
        try {
            ...
            $id = $request->get('id', '');
            $monthsCovered = max(1, (int) ($request->months_covered ?? 1));
            $monthLabel = $this->buildMonthLabel($request->month, $monthsCovered);
            ...
            $invoice->month            = $this->buildMonthLabel($request->month, $monthsCovered);
            ...
            $invoice->amount = $totalAmount * $monthsCovered;
            $invoice->tax_amount = $totalTax * $monthsCovered;
            $invoice->save();
    ...
    private function buildMonthLabel(string $startMonthName, int $monthsCovered): string
    {
        if ($monthsCovered === 12) {
            return 'Full Year ' . date('Y');
        }
        if ($monthsCovered > 1) {
            $monthNames = array_values(month());
            $startIndex = array_search($startMonthName, $monthNames);
            if ($startIndex !== false) {
                $endIndex = ($startIndex + $monthsCovered - 1) % 12;
                return $startMonthName . ' - ' . $monthNames[$endIndex];
            }
        }
