...
class KycVerificationService
{
    ...
    public function getAllByTenantId($id)
    {
        $kycVerifications = KycVerification::where('kyc_verifications.tenant_id', $id)
            ...
            ->join('property_units', 'tenants.unit_id', '=', 'property_units.id')
            ->whereNull('users.deleted_at')
            ->select('kyc_verifications.*', 'kyc_configs.name as config_name', 'kyc_configs.category', 'kyc_configs.is_both', 'users.first_name', 'users.last_name', 'properties.name as property_name', 'property_units.unit_name')
            ->get();
        foreach ($kycVerifications as $kycVerification) {
    ...
    public function statusChange($id)
    {
        try {
            ...
            // WhatsApp: notify tenant of KYC approval
            try {
                $tenant = Tenant::with('user')->find($kycVerification->tenant_id);
                $phone  = $tenant?->user?->whatsapp_number ?? null;
                if ($phone) {
                    app(WhatsAppService::class)->send(
                        $phone,
                        "✅ *KYC Approved*\n\nYour identity verification has been accepted. You now have full access to tenant services."
                    );
                }
            } catch (\Throwable $waEx) {
                Log::warning('WhatsApp KYC accept notification failed: ' . $waEx->getMessage());
            }
    ...
    public function rejectReasonStore($request)
    {
        ...
        try {
            ...
            // WhatsApp: notify tenant of KYC rejection
            try {
                $tenant = Tenant::with('user')->find($kycVerification->tenant_id);
                $phone  = $tenant?->user?->whatsapp_number ?? null;
                if ($phone) {
                    ...
                    app(WhatsAppService::class)->send(
                        $phone,
                        "❌ *KYC Rejected*\n\nYour identity verification was not accepted.{$reason}\n\nPlease re-submit the correct documents."
                    );
                }
            } catch (\Throwable $waEx) {
                Log::warning('WhatsApp KYC reject notification failed: ' . $waEx->getMessage());
            }
