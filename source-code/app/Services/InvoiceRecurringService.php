...
class InvoiceRecurringService
{
        ...
        return datatables($invoiceRecurring)
            ...
            ->addColumn('type', function ($invoiceRecurring) {
                ...
                if ($invoiceRecurring->recurring_type == INVOICE_RECURRING_TYPE_MONTHLY) {
                    $mc = max(1, (int) ($invoiceRecurring->months_covered ?? 1));
                    if ($mc === 1) {
                        $type = '<h6>Monthly</h6>';
                    } elseif ($mc === 12) {
                        $type = '<h6>Every 12 Months</h6>';
                    } else {
                        $type = '<h6>Every ' . $mc . ' Months</h6>';
                    }
                } elseif ($invoiceRecurring->recurring_type == INVOICE_RECURRING_TYPE_YEARLY) {
                    $inst = max(1, (int) ($invoiceRecurring->installments_per_year ?? 1));
                    if ($inst === 1) {
                        $type = '<h6>Yearly</h6>';
                    } else {
                        $type = '<h6>Yearly</h6><p>' . $inst . ' installments/year</p>';
                    }
    ...
    public function store($request)
    {
        ...
        try {
            ...
            $invoiceRecurring->months_covered = ($request->recurring_type == INVOICE_RECURRING_TYPE_MONTHLY)
                ? max(1, (int) ($request->months_covered ?? 1))
                : 1;
            $invoiceRecurring->installments_per_year = ($request->recurring_type == INVOICE_RECURRING_TYPE_YEARLY)
                ? (max(1, (int) ($request->installments_per_year ?? 1)) > 1 ? (int) $request->installments_per_year : null)
                : null;
