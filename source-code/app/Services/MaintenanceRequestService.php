...
class MaintenanceRequestService
{
    ...
    public function store($request)
    {
        ...
        try {
            ...
            $maintenance->priority = $request->priority ?? 2;
    ...
    public function statusChange($request)
    {
        ...
        try {
            ...
            // WhatsApp: notify tenant of maintenance status update
            try {
                $tenant = Tenant::with('user')
                    ->where('property_id', $maintenance->property_id)
                    ->where('unit_id', $maintenance->unit_id)
                    ->where('status', TENANT_STATUS_ACTIVE)
                    ->first();
                $phone = $tenant?->user?->whatsapp_number ?? null;
                if ($phone) {
                    $statusLabel = match ((int) $maintenance->status) {
                        MAINTENANCE_REQUEST_STATUS_INPROGRESS => 'In Progress 🔧',
                        MAINTENANCE_REQUEST_STATUS_COMPLETE   => 'Completed ✅',
                        default                               => 'Pending ⏳',
                    };
                    $msg = "🔔 *Maintenance Update*\n\nYour maintenance request status has changed to: *{$statusLabel}*";
                    if ($maintenance->amount) {
                        $msg .= "\n💰 Amount: " . number_format($maintenance->amount, 2);
                    }
                    if ($maintenance->resolved_date) {
                        $msg .= "\n📅 Resolved: " . $maintenance->resolved_date;
                    }
                    app(WhatsAppService::class)->send($phone, $msg);
                }
            } catch (\Throwable $waEx) {
                Log::warning('WhatsApp maintenance notification failed: ' . $waEx->getMessage());
            }
