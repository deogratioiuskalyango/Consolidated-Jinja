...
class NoticeBoardService
{
    ...
    public function store($request)
    {
        ...
        try {
            ...
            // WhatsApp: notify affected tenants on new notice creation (not updates)
            if (!$request->id) {
                try {
                    $wa = app(WhatsAppService::class);
                    $phones = \App\Models\User::whereIn('id', $tenants)
                        ->whereNotNull('whatsapp_number')
                        ->pluck('whatsapp_number')
                        ->toArray();
                    if ($phones) {
                        $msg  = "📢 *" . $noticeboard->title . "*\n\n" . $noticeboard->details;
                        $msg .= "\n\n🗓️ " . $noticeboard->start_date . " – " . $noticeboard->end_date;
                        $wa->sendBulk($phones, $msg);
                    }
                } catch (\Throwable $waEx) {
                    Log::warning('WhatsApp notice delivery failed: ' . $waEx->getMessage());
                }
            }
