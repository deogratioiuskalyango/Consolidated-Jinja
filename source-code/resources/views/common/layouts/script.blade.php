...
<script>
    ...
    // ── Modal fix ────────────────────────────────────────────────────────────
    // Admin templates commonly wrap all page content in a single container
    // (#layout-wrapper) that can have CSS transforms, stacking contexts, or
    // overflow clipping which prevents Bootstrap 5 fixed-position modals from
    // appearing. Moving every .modal element to <body> at DOM-ready is the
    // safest, zero-view-change solution and has no side-effects because
    // Bootstrap resolves modals by ID at the time a trigger is clicked.
    document.addEventListener('DOMContentLoaded', function () {
