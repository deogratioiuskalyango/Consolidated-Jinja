(function ($) {
    ...
    function applyRecurringTypeVisibility(modal, recurringType) {
        modal.find('.recurring_day').addClass('d-none');
        modal.find('.months_covered_field').addClass('d-none');
        modal.find('.installments_field').addClass('d-none');
        if (recurringType == 1) {
            modal.find('.months_covered_field').removeClass('d-none');
        } else if (recurringType == 2) {
            modal.find('.installments_field').removeClass('d-none');
        } else if (recurringType == 3) {
            modal.find('.recurring_day').removeClass('d-none');
        }
    }

    $(document).on('change', '.recurring_type', function () {
        applyRecurringTypeVisibility($(this).closest('.modal'), $(this).val());
    });
    ...
    function getDataEditRes(response) {
        ...
        selector.find('select[name=months_covered]').val(response.data.invoice.months_covered || 1)
        selector.find('select[name=installments_per_year]').val(response.data.invoice.installments_per_year || 1)
        selector.find('input[name=due_day_after]').val(response.data.invoice.due_day_after)
        applyRecurringTypeVisibility(selector, response.data.invoice.recurring_type)
    ...
    function getDetailsViewRes(response) {
        ...
        if (response.data.invoice.recurring_type == 1) {
            var mc = response.data.invoice.months_covered || 1;
            recurring = mc == 1 ? 'Monthly' : 'Every ' + mc + ' Months';
        } else if (response.data.invoice.recurring_type == 2) {
            var inst = response.data.invoice.installments_per_year || 1;
            recurring = inst == 1 ? 'Yearly' : 'Yearly (' + inst + ' installments)';
